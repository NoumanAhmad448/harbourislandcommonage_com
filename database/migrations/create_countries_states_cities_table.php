...
return new class extends Migration
{
    ...
    if (!Schema::hasTable(config("table.countries"))){
    ...
    if (!Schema::hasTable(config("table.states"))){

        Schema::create(config("table.states"), function (Blueprint $table) {
            $table->id();
    ...
    if (!Schema::hasTable(config("table.cities"))){
        Schema::create(config("table.cities"), function (Blueprint $table) {
            $table->id();
            $table->foreignId('country_id')->index()->constrained()->cascadeOnUpdate()->cascadeOnDelete();
    ...
    {
        Schema::dropIfExists(config("table.states"));
        Schema::dropIfExists(config("table.cities"));
